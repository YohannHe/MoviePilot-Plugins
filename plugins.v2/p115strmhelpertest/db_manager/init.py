...
def migration_db(db_path, script_location, version_locations: list):
    ...
    更新数据库
    """
    # 启动时的更新，调整从 /config/plugins/p115strmhelpertest/database/versions 中读取持久化的迁移脚本)
    meta_data_path = Path(settings.ROOT_PATH / 'app' / 'plugins' / 'p115strmhelpertest' / 'migration_meta.json')
...
def init_migration_scripts() -> bool:
    ...
    """
    # 使用 Path 对象定义路径
    source_path = Path(settings.ROOT_PATH / 'app' / 'plugins' / 'p115strmhelpertest' / 'database'/ 'versions')
    target_path = Path(settings.PLUGIN_DATA_PATH / 'p115strmhelpertest' / 'database' / 'versions')
