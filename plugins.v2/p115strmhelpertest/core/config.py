...
class ConfigManager(BaseModel):
    ...
    def _get_default_plugin_config_path() -> Path:
        ...
        返回默认的插件配置目录路径
        """
        return settings.PLUGIN_DATA_PATH / "p115strmhelpertest"
    ...
    def _get_default_plugin_database_script_location() -> Path:
        ...
        返回默认的插件数据库结构目录路径
        """
        return settings.ROOT_PATH / "app" / "plugins" / "p115strmhelpertest" / "database"
    ...
    # 插件名称
    PLUSIN_NAME: str = Field("P115StrmHelperTest", min_length=1)
    # 是否开启数据库WAL模式
