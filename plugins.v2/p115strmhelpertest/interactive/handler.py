    ...
    def handle_select_subscribe(
        ...
        try:
            ...
            if 0 <= item_index < len(search_data):
                ...
                resp = httpx.get(
                    f"{configer.get_config('moviepilot_address').rstrip('/')}/api/v1/plugin/P115StrmHelperTest/add_transfer_share?apikey={settings.API_TOKEN}&share_url={data.get('shareurl')}"
                )
